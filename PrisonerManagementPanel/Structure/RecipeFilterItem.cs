...
public class RecipeFilterItem : IExposable
{
    ...
    // 根据特定种族获取SelectedParts
    public List<BodyPartRecord> GetSelectedPartsWithRace(ThingDef race)
    {
        ...
        if (_selectedPartLabels == null)
        {
            ...
            return _selectedPartDefs
                .Where(def => def != null)
                .Select(def => BodyPartUtils.GetAllBodyPartsForRace(race)
        ...
        for (int i = 0; i < _selectedPartDefs.Count; i++)
        {
            if (_selectedPartDefs[i] != null && i < _selectedPartLabels.Count)
            {
                ...
                // 根据标签查找确切的身体部位记录
                var partRecord = BodyPartUtils.GetAllBodyPartsForRace(race)
                    .FirstOrDefault(record => record.def == partDef && record.LabelCap == partLabel);
                
                // 如果找不到确切匹配的部位，退回到只匹配BodyPartDef
                if (partRecord == null)
                {
                    partRecord = BodyPartUtils.GetAllBodyPartsForRace(race)
    ...
    public void SetSelectedParts(List<BodyPartRecord> parts)
    {
        if (parts == null)
        {
        ...
        {
            _selectedPartDefs = parts.Select(record => record.def).ToList();
            _selectedPartLabels = parts.Select(record => record.LabelCap).ToList();
        }
    }
